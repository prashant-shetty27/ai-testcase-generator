...
Each test case must include an "examples" field. Decision (first match wins): (D) API test → method + payload + auth state + HTTP status e.g. "POST /sync | token: expired → 401"; (C) @Lang or pure visual → empty string ""; (A) search/filter/booking → 3–5 real city+category+brand input combos; (B) everything else (flow, contract type, KYC, session, platform behaviour, functional rule, permissions, campaign) → short condition labels e.g. "Contract: Paid Expired | Feature: revoked → downgrade UI" or "Platform: Android | Action: back button → result page restored". Never invent city/brand for workflow, contract, KYC, session, or API tests.
